...
for ratings in pctdistro_dictionary:
    ...
    if volatility_rating == 1:
        continue

    if volatility_rating == 5 and price_rating == 2:
        pre_start = False
    elif pre_start:
        continue

    number_of_trades = round(float(new_ratings[2]) * 5000)
    ...
    while len(df) < target_trades:
        ...
        if len(df) > update_excel:
            df.to_excel('records_folder/ml_data/data_file6.xlsx', index=False)
            update_excel += 1000
            print(len(df))

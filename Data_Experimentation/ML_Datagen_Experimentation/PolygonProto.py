...
def main_flag_func(five_days, test_date):
    for ticker in ticker_list:

        try:
            price_by_min = []
            for ohlc in client.list_aggs(ticker, 5, 'minute', test_date, test_date, adjusted=True):
                price_by_min.append(ohlc.close)
        except:
            continue
        ...
        for ohlc in client.list_aggs(ticker, 5, 'minute', five_days, test_date, adjusted=True):
        ...
        for current_price in price_by_min:
            ...
            rsi_result = check_rsi(price_index, close_list)
            ttm_result = check_ttm(price_index, open_list, high_list, low_list, close_list, volume_list)
...
def main_watch_func(ticker, flag_price, flag_timer, list_index, buy_rsi, buy_ttm, test_date):
    price_by_1min = []
    for ohlc in client.list_aggs(ticker, 1, 'minute', test_date, test_date, adjusted=True):
        price_by_1min.append(ohlc.close)
    ...
    if len(price_by_1min) != 389:
        ...
        new_time_by_min = []
        for ohlc in client.list_aggs(ticker, 1, 'minute', test_date, test_date, adjusted=True):
            new_time_by_min.append(ohlc.timestamp)

        reformatted_time = []
        for t in new_time_by_min:
            dt = datetime.fromtimestamp(t / 1000)
            reformatted_time.append(str(dt))

        time_index = 0
        for timest in reformatted_time:
            new2 = timest.split(' ')
            minutes_seconds = new2[1].split(':')
            time_final = (int(minutes_seconds[0]) * 60) + int(minutes_seconds[1]) - 570
            if time_final < 0 or time_final > 389:
                try:
                    price_by_1min.pop(time_index)
                    continue
                except IndexError:
        ...
        if float(current_price) >= float(buy_spacer * flag_price):
            if len(price_by_1min) == 389:
                new_row = {'Ticker': ticker, 'Flag Price': flag_price, 'Buy Price': float(current_price),
                           ...
                           'Time Between Flag and Buy': (time_in_1min(price_index) - flag_timer),
...
def rsi_values(close_list):

    try:
        dataf = pd.DataFrame()
        dataf['RSI'] = ta.RSI(14, close_list)
        rsi_list = dataf['RSI'].tail(80).tolist()
        return rsi_list
    except IndexError:
        return "invalid"


def ttm_squeeze_values(open_list, high_list, low_list, close_list, volume_list):
    ...
    dataf = pd.DataFrame()
    dataf['TTM'] = ta.TTM(20, input_values=ohlcv)
    ttm_values_list = []
    for ttm in dataf['TTM'].tail(81):
        if ttm is None:
            return "invalid"
...
def check_rsi(list_index, close_list):
    rsi_index = list_index + 2
    day_rsi = rsi_values(close_list)
    ...
    try:
        validity = True
        if day_rsi[rsi_index] is None or day_rsi[rsi_index - 1] is None or day_rsi[rsi_index - 2] is None:
            validity = False
        if validity:
...
def check_ttm(list_index, open_list, high_list, low_list, close_list, volume_list):
    ttm_index = list_index + 3

    day_ttm = ttm_squeeze_values(open_list, high_list, low_list, close_list, volume_list)
    if day_ttm == "invalid":
        return "invalid"
    validity = True
    for item in day_ttm:
...
def convert_to_excel():
    ...
    df1 = pd.read_excel('records_folder/ml_training_data.xlsx', sheet_name='Sheet1')
    combined_df = pd.concat([df1, df], ignore_index=True)

    # Write DataFrame to Excel
    combined_df.to_excel('records_folder/ml_training_data.xlsx', index=False)
...
def create_graphs(test_date):
    ...
    for x_value in x_list:
        ...
        plt.savefig('records_folder/' + 'normal_sl/' + str(normal_stop_loss_constant) + '/' + test_date + '/' +
                    ref_list[ref_index] + '.png')
        plt.clf()
        ref_index += 1
...
financial_holidays = []
with open('holidays.txt', 'r') as filehandle:
    for line in filehandle:
        ind_ticker = line[:-1]
        financial_holidays.append(ind_ticker)

current_date = start_date
while True:
    current_date_str = current_date.strftime("%Y-%m-%d")

    if current_date.weekday() < 5 and current_date_str not in financial_holidays:

        test_date_dt = datetime.strptime(current_date_str, '%Y-%m-%d')
        ...
        main_flag_func(five_business_days_before_str, current_date_str)
        convert_to_excel()
        df = pd.DataFrame(columns=df.columns)

    print(current_date_str + ' complete')
